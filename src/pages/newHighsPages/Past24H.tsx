...
export function Past24H() {
    ...
    const allStocks = useMemo(() => {
        if (!dashboardData?.past24H) return [];
        return dashboardData.past24H.map((stock: any) => ({
            symbol: stock.symbol,
            ...
            dayGrowth: (() => {
                ...
                const recentPast = dashboardData?.pastHighs?.find((s: any) =>
                    s.symbol === stock.symbol &&
                    new Date(s.created_at) >= twentyFourHoursAgo
                );

                if (recentPast) {
                    const pastPrice = parseFloat(recentPast.price);
            ...
            industry: 'Tech', // Replace if real data is available
        }));
    }, [dashboardData]);
                ...
                <header className="border-b">
                    <div className="container mx-auto p-4 md:p-6">
                        <div className="flex items-center justify-between relative">
                            ...
                            <h1 className="text-lg font-semibold">Past 24 Hours</h1>
