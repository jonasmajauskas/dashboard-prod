...
export function AllNewHighs() {
  ...
  const allStocks = useMemo(() => {
    if (!dashboardData?.todayshighs) return [];
    return dashboardData.todayshighs.map((stock: any) => ({
      symbol: stock.symbol,
      name: stock.companyName || stock.name || stock.symbol,
      price: parseFloat(stock.price),
      dayGrowth: (() => {
        const past = dashboardData?.pastHighs?.find((s: any) =>
          s.symbol === stock.symbol && new Date(s.created_at) < new Date(Date.now() - 24 * 60 * 60 * 1000)
        );
        if (past) {
          const pastPrice = parseFloat(past.price);
          const growth = ((parseFloat(stock.price) - pastPrice) / pastPrice) * 100;
      ...
      industry: 'Tech', // Replace with actual industry if available
    }));
  }, [dashboardData]);
  ...
  const sortedStocks = [...filteredStocks].sort((a, b) => {
    ...
    return sortOrder === 'asc' ? a.dayGrowth - b.dayGrowth : b.dayGrowth - a.dayGrowth;
  });
        ...
        <header className="border-b">
          <div className="container mx-auto p-4 md:p-6">
            <div className="flex items-center justify-between relative">
              ...
              <h1 className="text-lg font-semibold">Today's New Highs</h1>
